...
class heGUI:
    ...
    def __init__(self, window):
        ...
        self.output_button = Button(window, text= "Select", command=lambda : self.select_file(self.output_entryText))
        ...
        # Run Napari
        self.napari_optical_button = Button(window, text = "Place landmarks on optical image", width = 30, command=lambda : self.optical_place_landmarks())
        self.napari_optical_button.grid(column = col_0, columnspan=3, row = self.row)

        self.row = self.row + 1


        self.napari_he_button = Button(window, text = "Place landmarks on H&E image", width = 30, command=lambda : self.he_place_landmarks())
        self.napari_he_button.grid(column=col_0, columnspan=3, row = self.row)
    ...
    def optical_place_landmarks(self):
    ...
    def he_place_landmarks(self):
        ...
        try:
            ...
            target_image = he_script.resize_(self.source_image, target_image)
            ...
            messagebox.showerror(title="Transformibi [target]", message="No optical image file opened")
        return
    ...
    def generate_json(self):
        ...
        # fov_size = self.fov_combobox.get()

        i=0
        for child in self.patient_order_treeview.get_children():
            print(self.patient_order_treeview.item(child)['values'][0])
            i = i + 1
